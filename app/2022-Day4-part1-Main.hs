...
main = do
  ...
  putStrLn ("total fully contained " ++ show total)
...
data Included = NotIncluded | Fully deriving (Enum) 
...
pairsAreIncluded ((pa1,pa2),(pb1,pb2)) = 
  if 
   (boundariesPositive) || (boundariesNegative) 
  then Fully else NotIncluded
 where boundariesPositive = (pb1 - pa1 >= 0) && (pa2 - pb2) >= 0
       boundariesNegative = (pb1 - pa1 <= 0) && (pa2 - pb2) <= 0
...
test = computeTotal ["2-4,6-8", "2-3,4-5", "5-7,7-9", "2-8,3-7", "6-6,4-6", "2-6,4-8"] == 2 