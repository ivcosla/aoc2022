...
main = do 
    ...
    let (_,_,rootSize) = head $ T.flatten fs
    let unusedSpace = 70000000 - rootSize
    let deleteRequirement = 30000000 - unusedSpace
    let result = foldl (findMoreThan deleteRequirement) [] $ T.flatten fs 
    let sorted = sortOn (\(_,_,size) -> size) result
...
testMoreThan = (foldl (findMoreThan 8381165) [] $ T.flatten testTree) == [(Dir,"/",48381165),(Dir,"d",24933642)]
